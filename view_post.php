...
if(isset($_POST['delete_review'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Đã xóa bài đánh giá!';
   }else{  
...
<section class="view-post">
   ...
   <div class="row">
      <div class="col">
         <img src="uploaded_img/<?= $fetch_post['image_01']; ?>" alt="" class="image">
         <div class="name"><?= $fetch_post['name']; ?></div>
         <div class="price"><span>Giá: </span><?= number_format($fetch_post['price'], 0, ',', '.'); ?><span> VND</span></div>
      </div>
      <div class="col">
         <div class="flex">
   ...
   <?php
         ...
         echo '<p class="empty">bài viết bị thiếu!</p>';
      }
   ?>
...
<section class="reviews-container">
    ...
    <div class="heading-view">
        ...
        <a href="add_review.php?get_id=<?= $get_id; ?>" class="view-btn" style="margin-top: 0;">Thêm đánh giá</a>
    </div>
   ...
   <div class="box" <?php if($fetch_review['user_id'] == $user_id){echo 'style="order: -1;"';}; ?>>
      ...
      <?php if($fetch_review['user_id'] == $user_id){ ?>
         <form action="" method="post" class="flexview-btn">
            <input type="hidden" name="delete_id" value="<?= $fetch_review['id']; ?>">
            <a href="update_review.php?get_id=<?= $fetch_review['id']; ?>" class="inline-option-btn">chỉnh sửa đánh giá</a>
            <input type="submit" value="xóa đánh giá" class="inline-delete-btn" name="delete_review" onclick="return confirm('bạn có chắc chắn xóa đánh giá không?');">
         </form>
      <?php }; ?>   
   </div>
   <?php
